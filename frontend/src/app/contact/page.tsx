...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://kajalproductsnew-backend.onrender.com", { 
        method: "POST",
      ...
      <motion.div className="bg-pink-100 shadow-lg rounded-2xl p-8 w-full max-w-lg border border-pink-300">
        <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
          ...
          <textarea
            ...
            className="px-4 py-2 rounded-lg border border-pink-300 focus:ring-2 focus:ring-pink-500 text-black"
          />
          <motion.button type="submit" className="px-6 py-3 rounded-xl bg-pink-500 text-white font-bold hover:bg-pink-600 transition-all">
